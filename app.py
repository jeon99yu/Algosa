    ...
    if not products.empty:
        ...
        if not reviews_df.empty:
            ...
            with st.expander("🗂️ 선택된 상품 리뷰 테이블 보기", expanded=False):
                ...
                _df = _df.rename(
                    columns={"userNickName": "닉네임", "content": "내용", "grade": "평점", "createDate": "작성일"}
                )
                st.dataframe(_df, use_container_width=True, hide_index=True)

            st.markdown("### 📊 리뷰 분석 결과")

            reviews_df["sentiment"] = reviews_df["grade"].apply(lambda g: "긍정" if g >= 4 else ("부정" if g <= 2 else "중립"))
            ...
            with c1:
                ...
                if sum(values) > 0:
                    ...
                    ax.legend(wedges, legend_labels, loc="center left", bbox_to_anchor=(1.02, 0.5), frameon=False, borderaxespad=0.0)
                    ax.set(aspect="equal")
                    plt.tight_layout()
